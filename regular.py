...
if __name__ == '__main__':
    ...
    for line in sys.stdin:
        ...
        try:
            ...
            continue
        src_ip, dst_ip = fields[1], fields[2][:-1]
        #print(line)
        ...
        update_table(hip_src, hip_pair, src_ip, dst_ip, int(time_diff.total_seconds()//3600))
        if args.daily:
            update_table(dip_src, dip_pair, src_ip, dst_ip, time_diff.days)
        if args.weekly:
            update_table(wip_src, wip_pair, src_ip, dst_ip, time_diff.days//7)
    chi_sqtest(hip_src, hip_pair, int(time_diff.total_seconds()//3600), 'hour')
    if args.daily:
